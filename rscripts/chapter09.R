...
# 단계 3: 레코드 삽입(insert)
query = "insert into student values('s4', 'lee', 'ulsan', 22)"
...
query = "update student set age = 40 where sname = 'hong'"
dbSendUpdate(conn, query)
# 수정된 레코드 조회
query = "select * from student where sname = 'hong'"
dbGetQuery(conn, query)

# 단계 6: 레코드 삭제 - name이 '홍길동'인 레코드 삭제
query = "delete from student where sname = 'kim'"
dbSendUpdate(conn, query)
